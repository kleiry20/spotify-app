...
export const ListingScreen = (props) => {
  ...
  const updateRating = (rate, song, rating) => {
    ...
    axios.put(
      process.env.REACT_APP_API_URL + 'ratings/update/'+rating.id+'/', new_rating)
    .then(res => {
      alert('success');
      setFetchArtist(false)
      setFetchSong(false)
  ...
  function RenderRating({ song }) {
    ...
    useEffect(() => {
      function getRating(id) {
        axios.get(process.env.REACT_APP_API_URL + 'ratings/fetch/'+user_id+'/'+id+'/')
        .then(response => {
    ...
    if (fetch_song){
      return(
        songs.map((song, index) => {
          ...
          return <tr>
          <td style={{width: '300px'}}> <Image src={process.env.REACT_APP_API_URL_NOHASH +song.image} thumbnail='true' /></td>
          <td>{song.name}</td>
          <td>{song.artist_name}</td>
          <td><Rating ratingValue={avg_rat} readonly='true' /* Available Props */ /></td>
